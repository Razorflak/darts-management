<script lang="ts">
	...
	import { createTournament } from '../utils.js'
...
<div class="flex flex-wrap items-center gap-2 overflow-x-auto pb-1">
	{#each tournaments as t}
		<div class="group flex shrink-0 items-center gap-1">
			<Button
				...
				title={t.name || 'Nouveau tournoi'}
			>
				{t.name || 'Nouveau tournoi'}
				{#if tournaments.length > 1}
					<button
						type="button"
						onclick={(e) => { e.stopPropagation(); removeTournament(t.id) }}
						class="ms-1.5 -me-1 rounded-full p-0.5 hover:bg-black/10 transition-colors"
	...
	<Button size="sm" color="alternative" pill outline onclick={addTournament} title="Ajouter un tournoi">
