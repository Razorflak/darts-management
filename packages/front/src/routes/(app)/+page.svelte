...
{#if data.user && !data.player}
	<div
		class="mb-6 rounded-xl border border-dashed border-orange-200 bg-orange-50/40 p-5"
	>
		<div
			class="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-white shadow-sm ring-1 ring-orange-100"
		>
		...
		<a
			href="/profile/create"
			class="mt-3 inline-block text-xs font-medium text-orange-700 underline"
		>
			Créer mon profil →
		</a>
	</div>
...
<section class="mb-10">
	...
	>
		<span
			class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary-100 text-primary-600"
		>
	...
	{#if openEvents.length === 0}
		<div
			class="rounded-xl border border-dashed border-gray-200 bg-gray-50 px-6 py-10 text-center"
		>
		...
		<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
			{#each openEvents as { event, tournaments } (event.id)}
				...
				<a
					...
					class="flex items-center gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-[0_1px_3px_0_rgb(0_0_0/0.06)] transition-shadow hover:shadow-[0_4px_6px_-1px_rgb(0_0_0/0.08)]"
				>
					<div class="min-w-0 flex-1">
						<h3 class="font-semibold leading-tight text-gray-900">
							{event.name}
						</h3>
...
<section>
	<h2
		class="mb-4 flex items-center gap-2 text-base font-semibold text-gray-400"
	>
		<span
			class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-100 text-gray-400"
		>
	...
	<div class="grid gap-4 sm:grid-cols-3">
		<div
			class="rounded-xl border border-dashed border-gray-200 bg-gray-50/40 p-5"
		>
			<div
				class="mb-3 flex h-9 w-9 items-center justify-center rounded-lg bg-white shadow-sm ring-1 ring-gray-100"
			>
			...
			<p class="mt-1 text-xs text-gray-400 leading-relaxed">
				Actualités, résultats et classements de votre comité régional.
			</p>
		</div>
		...
		>
			...
			<p class="mt-1 text-xs text-gray-400 leading-relaxed">
				Suivi de votre progression, calendrier et classement en temps réel.
			</p>
		</div>
		...
		>
			...
			<p class="mt-1 text-xs text-gray-400 leading-relaxed">
				Vos résultats, statistiques et performances sur tous vos tournois.
			</p>
		</div>
	</div>
</section>
