...
async function findOrCreateTeamInternal(
	sql: Sql,
	playerIds: string[],
): Promise<string> {
...
const internalRepoTeam = {
	findOrCreate: async (sql: Sql, playerIds: string[]): Promise<string> =>
		sql.begin(async (rawTx) =>
			findOrCreateTeamInternal(rawTx as unknown as Sql, playerIds),
		),
}

export const teamRepository = createRepository(defaultSql, internalRepoTeam)
export const getTeamRepositoryWithSql = (sql: Sql) =>
	createRepository(sql, internalRepoTeam)
