...
must_haves:
  ...
  key_links:
    ...
    - from: "+page.svelte PlayerSearch component"
      ...
      via: "fetch GET /admin/events/[id]/tournaments/[tid]/players/search?q="
...
<!-- PlayerSearch component:
  ...
  Props: tournamentId: string, onSelect: (p: PlayerSearchResult) => void
  The component fetches from /tournaments/${tournamentId}/admin/players/search?q=
  For the new admin route, we need a new search endpoint at the new path.
-->
...
<task type="auto">
  <name>Task 2: Admin roster UI page with status management buttons</name>
  <files>
    packages/front/src/routes/(admin)/admin/events/[id]/tournaments/[tid]/+page.svelte
  </files>
  <action>
Create the roster management page. Adapts the existing `/tournaments/[id]/admin/+page.svelte` UI but:
1. Uses new route paths for all fetch calls (`/admin/events/${params.id}/tournaments/${data.tournament.id}/...`)
2. Adds tournament status management buttons at the top
3. Uses `$page.params` to construct correct URLs
...
<Card class="mt-2">
  ...
  <!-- Player search uses the new (admin) search endpoint -->
  <PlayerSearch
...
**Important note about PlayerSearch component:** Check the existing `PlayerSearch.svelte` component signature at `packages/front/src/lib/tournament/components/PlayerSearch.svelte`. If it hardcodes the search URL as `/tournaments/${tournamentId}/admin/players/search`, you need to add a `searchUrl` prop to make it configurable. If it already has a `searchUrl` prop, use it directly. If it does not, update the component to accept an optional `searchUrl` prop that defaults to the existing URL pattern:

```svelte
<!-- In PlayerSearch.svelte — add prop if missing: -->
interface Props {
  tournamentId: string
  searchUrl?: string  // optional override
  onSelect: (p: PlayerSearchResult) => void
}
let { tournamentId, searchUrl, onSelect }: Props = $props()
// Use: searchUrl ?? `/tournaments/${tournamentId}/admin/players/search`
```

After writing the page, run svelte-autofixer MCP tool to check for Svelte 5 issues. Fix any reported problems. Then run typecheck:
```bash
cd /home/jta/Projects/darts-management/master && pnpm typecheck 2>&1 | tail -20
```
  </action>
  ...
  <done>Page `/admin/events/[id]/tournaments/[tid]` renders roster with check-in buttons, status badges, status transition buttons, and player search/add section; typecheck passes.</done>
</task>
...
Manual check: visit `/admin/events/[event-id]` — see event detail + tournament list. Click "Gérer le roster" on a tournament — see roster at `/admin/events/[event-id]/tournaments/[tournament-id]`. Status buttons allow forward/backward transitions.
</verification>
...
- Player search autocomplete works via new `/admin/.../players/search` endpoint
